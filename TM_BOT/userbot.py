...
async def main():
    async with app:
        print("\n" + "="*60)
        print("🔄 Verifying group access...")
        print("="*60)
        
        # ✅ CHECK GROUP ACCESS FIRST
        if not await verify_group():
            print("\n❌ Bot cannot proceed. Make sure you're a member!")
            return
        
        print("\n" + "="*60)
        print("🔥 USERBOT RUNNING 24/7")
        print("="*60)
        print("✅ Auto-saving new movies in real-time")
        print("✅ Smart re-indexing every 6 hours")
        print("✅ MongoDB: Connected")
        print("="*60)

        await asyncio.gather(
            periodic_index(),                    # Smart indexing every 6 hours
            asyncio.Event().wait()               # Keep live @app.on_message running
        )
